...
from Densenet.model import densenet121_pre
...
def main():
    ...
    net3 = densenet121_pre(num_classes=10).to(device)

    # load pretrain weights
    model_weight_path = "./Densenet/densenet121-a639ec97.pth"
    ...
    x1 = torch.load(model_weight_path1, map_location=device)
    net1.load_state_dict(x1, strict=False)
...
def test(show=False):
    ...
    validate_loader = torch.utils.data.DataLoader(validate_dataset,
                                                  ...
                                                  num_workers=nw)
    net = densenet121(num_classes=10).to(device)
    model_weight_path = "./checkpoints/767epoch_0.912.pth"
    assert os.path.exists(model_weight_path), "file {} does not exist.".format(model_weight_path)
    net.load_state_dict(torch.load(model_weight_path, map_location=device), strict=False)
    ...
    if show == True:
        ...
        plt.xlabel('Label')
        # plt.savefig('./figures/useless.pdf', dpi=200, bbox_inches='tight')
